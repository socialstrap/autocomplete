...
;(function ( $, window, document, undefined ) {
	$.widget("ui.triggeredAutocomplete", $.extend(true, {}, $.ui.autocomplete.prototype, {
		...
		_create:function() {
			...
			if ($(this.element).attr('id') == 'term') {
				self.options.topSearch = true;
			}
		...
		_renderItem:  function( ul, item ) {
			...
			if (this.options.topSearch == true && item.link != undefined) {
				user_link = 'href="' + item.link + '"';
			}
			
			var user_html = "<div class='user-autocomplete'><a " + user_link + "><div><img src='" + item.avatar + "' /><span>"+item.name+"</span></div></a></div>";
			
			if (this.options.topSearch == true) {
				return $( "<li></li>" )
		...
		search: function(value, event) {
			...
			if (this.options.topSearch == true && contents.length >= this.options.minLength) {
				
				this.options.source = this.options.sources['@'];
				this._initSource();
				this.updateHidden();
				return this._search(contents);
			}

			
			// alcalbg: double trigger
			...
			// alcalbg: multi trigger datasource
			...
			// old: var regex = new RegExp('\\B\\'+this.options.trigger+'([\\w\\-]+)');
			// alcalbg: for non latin hashtags
			var regex = new RegExp('\\B\\'+this.options.trigger+'([\\S\\-]+)');

			// alcalbg: one word only please: 
			// old: if (contents.indexOf(this.options.trigger) >= 0 && check_contents.match(regex)) {
			if (contents.indexOf(this.options.trigger) >= 0 && check_contents.match(regex) && check_contents.indexOf(' ') <= 0) {
				...
				if (term.length > 0 && (term.length >= this.options.minLength && term.length <= this.options.maxLength)) {
